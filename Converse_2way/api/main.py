...
class MessageCreate(BaseModel):
    content: str
    # UI language, so the AI answers in the language the user is reading.
    lang: str = "ja"
...
def _reply_to_message(conversation_id, content, lang="ja"):
    ...
    system_prompt = build_system_prompt(resident, lang)
...
def send_text_message(conversation_id: int, payload: MessageCreate):
    reply = _reply_to_message(conversation_id, payload.content, payload.lang)
    return {"reply": reply}


@app.post("/conversations/{conversation_id}/messages/audio")
async def send_audio_message(
    conversation_id: int, file: UploadFile = File(...), lang: str = "ja"
):
    audio_bytes = await file.read()
    transcribed_text = transcribe_audio(
        audio_bytes, filename=file.filename or "audio.webm", lang=lang
    )
    reply = _reply_to_message(conversation_id, transcribed_text, lang)
...
async def voice_offer(conversation_id: int, payload: VoiceOffer):
    ...
    asyncio.create_task(run_voice_bot(connection, conversation_id, resident, payload.lang))

    return {"sdp": answer["sdp"], "type": answer["type"]}


@app.post("/conversations/{conversation_id}/extract")
def extract(conversation_id: int, lang: str = "ja"):
    messages = get_messages(conversation_id)
    if not messages:
        raise HTTPException(status_code=400, detail="no messages to extract")
    result = extract_conversation(messages, lang)
    save_extraction(
...
@app.get("/reports")
def get_reports(date: str, resident_id: int | None = None, lang: str = "ja"):
    ...
    for rid, resident_records in records_by_resident.items():
        ...
        report_text = generate_report(resident_name, resident_records, lang)
