...
export default function ReportsPage() {
    ...
    try {
      const query =
        selectedResidentId === 0
          ? `?date=${date}`
          : `?date=${date}&resident_id=${selectedResidentId}`;
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 p-6">
      <h1 className="text-xl font-medium text-neutral-900">申し送りレポート</h1>
      ...
      {reports?.map((r) => (
        <section key={r.resident_id} className="flex flex-col gap-2 rounded border border-neutral-200 p-4">
          <div className="flex items-center justify-between">
            ...
            >
              {copiedId === r.resident_id ? "コピーしました" : "コピー"}
            </button>
          </div>
          <textarea
